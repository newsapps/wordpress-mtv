...
namespace chicagonow\template\functions;
use mtv\shortcuts;
...
$twig->addFunction('the_post_meta', new Twig_Function_Function('chicagonow\template\functions\get_the_post_meta'));
$twig->addFunction('wpautop', new Twig_Function_Function('wpautop'));
$twig->addFunction('comment_form', new Twig_Function_Function('chicagonow\template\functions\comment_form'));
...
$twig->addFunction('formatted_date', new Twig_Function_Function('chicagonow\template\functions\formatted_date'));
$twig->addFunction('relative_time', new Twig_Function_Function('chicagonow\template\functions\relative_time'));
...
$twig->addFunction('has_post_thumbnail_caption', new Twig_Function_Function('chicagonow\template\functions\has_post_thumbnail_caption'));
$twig->addFunction('get_the_post_thumbnail', new Twig_Function_Function('get_the_post_thumbnail'));
$twig->addFunction('get_the_post_thumbnail_caption', new Twig_Function_Function('chicagonow\template\functions\get_the_post_thumbnail_caption'));
$twig->addFunction('get_thumbnail_image_src', new Twig_Function_Function('chicagonow\template\functions\get_thumbnail_image_src'));
$twig->addFunction('get_featured_image_src', new Twig_Function_Function('chicagonow\template\functions\get_featured_image_src'));

// Galleries
$twig->addFunction('get_post_gallery', new Twig_Function_Function('chicagonow\shortcuts\get_gallery_attachments'));
...
$twig->addFunction('get_search_form', new Twig_Function_Function('get_search_form'));
$twig->addFunction('get_bloginfo', new Twig_Function_Function('get_bloginfo'));
$twig->addFunction('get_blog_avatar', new Twig_Function_Function('get_blog_avatar'));
$twig->addFunction('get_categories', new Twig_Function_Function('chicagonow\template\functions\get_categories'));

// Template functions
$twig->addFunction('wp_head', 
    new Twig_Function_Function('chicagonow\template\functions\get_wp_head', array('is_safe'=>array('html')))
);
$twig->addFunction('wp_footer', 
    new Twig_Function_Function('chicagonow\template\functions\get_wp_footer', array('is_safe'=>array('html')))
);
$twig->addFunction('body_class', 
    new Twig_Function_Function('chicagonow\template\functions\get_body_class', array('is_safe'=>array('html')))
);
$twig->addFunction('language_attributes', 
    new Twig_Function_Function('chicagonow\template\functions\get_language_attributes', array('is_safe'=>array('html')))
);
$twig->addFunction('get_header', 
    new Twig_Function_Function('chicagonow\template\functions\get_header', array('is_safe'=>array('html')))
);
$twig->addFunction('get_footer', 
    new Twig_Function_Function('chicagonow\template\functions\get_footer', array('is_safe'=>array('html')))
);
$twig->addFunction('get_sidebar', 
    new Twig_Function_Function('get_sidebar', array('is_safe'=>array('html')))
);
$twig->addFunction('dynamic_sidebar', 
    new Twig_Function_Function('chicagonow\template\functions\dynamic_sidebar', array('is_safe'=>array('html')))
);
$twig->addFunction('sidebar_is_populated', 
    new Twig_Function_Function('chicagonow\template\functions\sidebar_is_populated', array('is_safe'=>array('html')))
);
$twig->addFunction('get_blog_category', 
    new Twig_Function_Function('chicagonow\template\functions\get_blog_category', array('is_safe'=>array('html')))
);
$twig->addFunction('omniture_tags', 
    new Twig_Function_Function('chicagonow\theme\functions\omniture_tags', array('is_safe'=>array('html')))
);

$twig->addFunction('this_year', new Twig_Function_Function('chicagonow\template\functions\this_year'));
...
$twig->addFunction('has_nav_menu', 
    new Twig_Function_Function('chicagonow\template\functions\has_nav_menu', array('is_safe'=>array('html'))));
...
function get_blog_category($blogid) {
    global $entity_manager;

    $blog = $entity_manager->find('Entities\Blog', $blogid);
    $blog_cats = $blog->getCategories();
    $blog_cat = $blog_cats[0];
 ...
 **/
add_filter('get_avatar', 'chicagonow\template\functions\get_fb_avatar', 20, 5);
function get_fb_avatar($avatar, $id_or_email, $size, $default, $alt) {

    // Return if we already have an avatar that's not from gravatar
    if ( !empty($avatar) && !strpos($avatar, 'gravatar') ) return $avatar;

    // error_log($id_or_email);
    ...
    $fb_img = get_usermeta($user_id, 'userthumb');

    //If so, replace the avatar! Otherwise, fallback on what WP core already gave us.
    if ($fb_img != '') {
        $avatar = "<img alt='fb_avatar' src='$fb_img' class='avatar avatar-{$size} photo' height='{$size}' width='{$size}' />";    
    }
...
function get_categories() {
    global $entity_manager;
    $categories = $entity_manager->getRepository('Entities\Category')->findAll();

    usort($categories, function($x, $y) {
            $xnice = trim(strtolower($x->nicename()));
            $ynice = trim(strtolower($y->nicename()));
